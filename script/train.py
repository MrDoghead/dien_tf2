...
def test(
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if model_type == 'DNN':
            ...
            model = Model_DIN_V2_Gru_Vec_attGru_Neg(n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, data_type, batch_size=batch_size, max_length=maxlen)
        else:
            print ("Invalid model_type : %s", model_type)
            return
        # for var in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
        ...
        if data_type == 'FP32':
            ...
            model.save(sess, "./dien_frozen", saved_model=True)
            exit(0)
        if data_type == 'FP16':
        ...
        """
        export_dir = "./dnn_saved_model"
        save_model_dir = os.path.join(export_dir,str(args.batch_size))
        if os.path.exists(save_model_dir):
            shutil.rmtree(save_model_dir)
        model.save(sess, save_model_dir, saved_model=True)
