...
class Model(object):
     ...
     max_length=100, device = 'gpu'):
        ...
        if synthetic_input:
            ...
            with tf.name_scope('Inputs'):
                self.mid_his_batch_ph = tf.compat.v1.placeholder(tf.int32, [batch_size, max_length], name='mid_his_batch_ph')
                self.cat_his_batch_ph = tf.compat.v1.placeholder(tf.int32, [batch_size, max_length], name='cat_his_batch_ph')
                self.uid_batch_ph = tf.compat.v1.placeholder(tf.int32, [batch_size, ], name='uid_batch_ph')
                self.mid_batch_ph = tf.compat.v1.placeholder(tf.int32, [batch_size, ], name='mid_batch_ph')
                self.cat_batch_ph = tf.compat.v1.placeholder(tf.int32, [batch_size, ], name='cat_batch_ph')
                self.mask = tf.compat.v1.placeholder(self.model_dtype, [batch_size, max_length], name='mask')
                self.seq_len_ph = tf.compat.v1.placeholder(tf.int32, [batch_size], name='seq_len_ph')
                self.target_ph = tf.compat.v1.placeholder(self.model_dtype, [batch_size,2], name='target_ph')
                self.lr = tf.compat.v1.placeholder(tf.float64, [])
                self.use_negsampling =use_negsampling
                if use_negsampling:
                    self.noclk_mid_batch_ph = tf.compat.v1.placeholder(tf.int32, [batch_size, max_length, 5], name='noclk_mid_batch_ph') #generate 3 item IDs from negative sampling.
                    self.noclk_cat_batch_ph = tf.compat.v1.placeholder(tf.int32, [batch_size, max_length, 5], name='noclk_cat_batch_ph')
    ...
    def save(self, sess, path, saved_model=False):
        if not saved_model:
            saver = tf.compat.v1.train.Saver()
            saver.save(sess, save_path=path)
        else:
            inps={
                    'Inputs/uid_batch_ph':self.uid_batch_ph,
                    'Inputs/mid_batch_ph':self.mid_batch_ph,
                    'Inputs/cat_batch_ph':self.cat_batch_ph,
                    'Inputs/mid_his_batch_ph':self.mid_his_batch_ph,
                    'Inputs/cat_his_batch_ph':self.cat_his_batch_ph,
                    'Inputs/mask':self.mask,
                    # 'Inputs/target_ph':self.target_ph,
                    'Inputs/seq_len_ph':self.seq_len_ph,
                    # 'Inputs/noclk_mid_batch_ph':self.noclk_mid_batch_ph,
                    # 'Inputs/noclk_cat_batch_ph':self.noclk_cat_batch_ph
                    }
            outs={
                    'dien/fcn/add_6':self.y_hat
                    }
            #method1
            ...
            #method3
            builder = tf.saved_model.builder.SavedModelBuilder("./dien_signature/")
            signature = tf.saved_model.signature_def_utils.build_signature_def(
                    inputs=inps, outputs=outs, method_name="dien_signature")
            builder.add_meta_graph_and_variables(sess=sess, tags=[tf.saved_model.tag_constants.SERVING],
                                     signature_def_map={"dien_signature": signature})
            builder.save()
